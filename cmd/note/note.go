...
import (
	...
	"github.com/roshbhatia/agent-notes/internal/store"
	"github.com/roshbhatia/go-utils/paths"
)

const usageText = `Agent review notes on a working tree. The owner's editor reads them back.
...
A write never opens anything. It publishes the record, and a reader that is
...
// The open notes, re-anchored, as the provider reads them. A store that is
// absent or empty is no notes; a store that does not parse is an error the
// dispatcher logs, because a silent pass there would hide the owner's question.
func openNotes() ([]json.RawMessage, error) {
	s, err := openStore()
	if err != nil {
		return nil, err
	}
	...
	doc, err := readDoc(s)
	if err != nil {
		return nil, fmt.Errorf("%s is not a valid note store", s.Path)
	}
	return keepOpen(reanchor(doc.Notes)), nil
}
...
func renderOpen(notes []json.RawMessage) string {
	if len(notes) == 0 {
		return ""
	}
	var b strings.Builder
	fmt.Fprintf(&b, "The owner left %d note(s) on the diff waiting for an answer.\n", len(notes))
	for _, raw := range notes {
		var cur existing
		if err := json.Unmarshal(raw, &cur); err != nil {
			continue
	...
	b.WriteString("Read the code each one names, then answer it with `note answer --id <id> --summary <text>`.")
	return b.String()
}
